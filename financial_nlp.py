...
Original file is located at
    https://colab.research.google.com/drive/1TzjQMuF-YYnvRFpnj9ahowX7QIAKh44f
"""
...
import os
os.environ["GEMINI_API_KEY"] = "AIzaSyD-yHbXImrScEra8v6VuybpCqsnA2GWl2o"
...
def generate_response(prompt):
    ...
    # Debugging: Print out the full response for analysis
...
def finAgent(prompt):
    """Uses Gemini 1.5 Flash with Chain-of-Thought for financial reasoning."""
    cot_prompt = (
        f"Let's think step by step and perform the necessary calculations. If any data is missing, make reasonable assumptions. {prompt}"
    )
    return generate_response(cot_prompt)


def logAgent(prompt):
    """Uses Gemini 1.5 Flash with Chain-of-Thought for logical reasoning."""
    cot_prompt = f"Think logically step by step. {prompt}"
    return generate_response(cot_prompt)

# Function to determine which agent to use
def solve_question(question, label):
    if label == "financial":
        return finAgent(question)
    else:
        return logAgent(question)

example_question = """
A company reported $500M in revenue for Q1, $600M for Q2, $550M for Q3, and $650M for Q4 in 2023.
What is the total revenue for the year?
"""
example_label = "financial"

result = solve_question(example_question, example_label)
